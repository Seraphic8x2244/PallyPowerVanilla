...
- Branch head before this handoff update: `8867b533ea04e1891ab11526c5dc0be3bed7bd6f`
...
### Stage 7 - Post-Validation Naming / Reference Refactor — FIRST FIVE SLICES USER-VERIFIED
...
## Exact Next Step
Audit the remaining 28 `getglobal()` lines in `PallyPower.lua` and 2 in `PallyPowerUI.lua`, regroup them after the accepted fifth slice, and choose the smallest coherent repeated family for the sixth Stage 7 slice. Preserve uncertain compatibility globals and do not mix in behavior fixes for the legacy tooltip-refresh or player-button mouse-wheel observations. Keep callback modernization, `IsPally` / `PP_IsPally` cleanup, and RF/Judgement preset persistence deferred. Before handing off the next testable runtime state, bump the TOC numeric version and run the canonical real Lua 5.0.2 compiler check.
