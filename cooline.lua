...
local function RefreshSpellIconOptions()
	if not optionsFrame or not optionsFrame.spellIconMode then return end

	optionsFrame.updating = true
	optionsFrame.spellIconMode:SetValue(CoolineCharDB.spellIconOverride and 1 or 0)
...
local function RefreshItemIconOptions()
	if not optionsFrame or not optionsFrame.itemIconMode then return end

	optionsFrame.updating = true
	optionsFrame.itemIconMode:SetValue(CoolineCharDB.itemIconOverride and 1 or 0)
...
local function BuildOptions()
	...
	MakeText(page, "Bar Direction", 28, -222, 11, false)
	optionsFrame.direction = MakeBinarySlider(page, "Horizontal", "Vertical", 174, -224, 140)

	MakeText(page, "Icon Direction", 28, -258, 11, false)
	optionsFrame.iconDirection = MakeBinarySlider(page, "Ascending", "Descending", 174, -260, 140)

	MakeText(page, "Sizes", 18, -304, 12, true)
	optionsFrame.length = MakeValueSlider(page, "Bar Length", 28, -330, 170, 100, 1000, 10)
	optionsFrame.width = MakeValueSlider(page, "Bar Width", 28, -390, 170, 2, 100, 2)
	optionsFrame.oversize = MakeValueSlider(page, "Icon Oversize", 28, -450, 170, -50, 50, 2)

	MakeText(page, "Opacity", 282, -304, 12, true)
	optionsFrame.active = MakeValueSlider(page, "Bar Active", 292, -330, 130, 0, 100, 10)
	optionsFrame.inactive = MakeValueSlider(page, "Bar Inactive", 292, -410, 130, 0, 100, 10)
	...
	optionsFrame.spellIconMode = MakeBinarySlider(spells, "Use Appearance", "Custom", 174, -80, 140)
	optionsFrame.spellOversize = MakeValueSlider(spells, "Icon Oversize", 28, -110, 350, -50, 50, 2)

	MakeText(spells, "Filter Type", 18, -170, 12, true)
	optionsFrame.filterType = MakeBinarySlider(spells, "Blacklist", "Whitelist", 174, -196, 140)

	MakeText(spells, "Add Spell", 18, -236, 12, true)
	optionsFrame.spellAdd = MakeEditBox(spells, 28, -266, 330)
	optionsFrame.spellAdd:SetJustifyH("LEFT")
	optionsFrame.spellAdd:SetTextInsets(6, 6, 0, 0)

	addButton = MakeButton(spells, "Add", 70)
	addButton:SetPoint("TOPLEFT", spells, "TOPLEFT", 372, -265)

	MakeText(spells, "Filtered Spells", 18, -310, 12, true)

	listFrame = CreateFrame("Frame", nil, spells)
	listFrame:SetPoint("TOPLEFT", spells, "TOPLEFT", 28, -338)
	...
	optionsFrame.itemIconMode = MakeBinarySlider(items, "Use Appearance", "Custom", 174, -80, 140)
	optionsFrame.itemOversize = MakeValueSlider(items, "Icon Oversize", 28, -110, 350, -50, 50, 2)

	MakeText(items, "Filter Type", 18, -170, 12, true)
	optionsFrame.itemFilterType = MakeBinarySlider(items, "Blacklist", "Whitelist", 174, -196, 140)

	MakeText(items, "Add Item", 18, -236, 12, true)
	optionsFrame.itemAdd = MakeEditBox(items, 28, -266, 330)
	optionsFrame.itemAdd:SetJustifyH("LEFT")
	optionsFrame.itemAdd:SetTextInsets(6, 6, 0, 0)

	itemAddButton = MakeButton(items, "Add", 70)
	itemAddButton:SetPoint("TOPLEFT", items, "TOPLEFT", 372, -265)

	MakeText(items, "Filtered Items", 18, -310, 12, true)

	local itemListFrame = CreateFrame("Frame", nil, items)
	itemListFrame:SetPoint("TOPLEFT", items, "TOPLEFT", 28, -338)
	...
	optionsFrame.spellIconMode:SetScript("OnValueChanged", function()
		if optionsFrame.updating then return end

		CoolineCharDB.spellIconOverride = this:GetValue() >= 0.5
	...
	optionsFrame.spellOversize.edit:SetScript("OnEnterPressed", function()
		...
		CoolineCharDB.spellIconOversize = value
		optionsFrame.updating = true
		optionsFrame.spellOversize:SetValue(min(value, 50))
		optionsFrame.spellIconMode:SetValue(1)
		optionsFrame.updating = false
		ApplyVisualLayout()
		RefreshSpellIconOptions()
	end)
	...
	optionsFrame.itemIconMode:SetScript("OnValueChanged", function()
		if optionsFrame.updating then return end

		CoolineCharDB.itemIconOverride = this:GetValue() >= 0.5
	...
	optionsFrame.itemOversize.edit:SetScript("OnEnterPressed", function()
		...
		CoolineCharDB.itemIconOversize = value
		optionsFrame.updating = true
		optionsFrame.itemOversize:SetValue(min(value, 50))
		optionsFrame.itemIconMode:SetValue(1)
		optionsFrame.updating = false
		ApplyVisualLayout()
		RefreshItemIconOptions()
	end)
